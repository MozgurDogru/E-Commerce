...
namespace iakademi47_proje.Models
{
    public class Cls_User
    {
        ...
        //ctrl +M O ,ctrl M L  metod agacını aç-kapat
        ...
        public static bool SendSms(string OrderGroupGUID)
        {
            try
            {
                using (iakademi47Context context = new iakademi47Context())
                {
                    ...
                    string icerik = "Sayın " + user.NameSurname + " , " + DateTime.Now + " tarihinde " + OrderGroupGUID + " no lu siparişiniz alınmıştır.";
                    ...
                    if (result != "-1")
                    {
                        //sms gitti,Orders tablosunda sms kolonuna true bas

                    }
                    else
                    {
                        //sms gitmedi,Orders tablosunda sms kolonuna false bas
                        //ilgili admin personeline sms gönder
                        //ilgili admin personeline mail gönder
                    }

                    //Sayın Nergis DOĞRU,22092023 tarihinde 22092023194923 no lu siparişiniz alınmıştır.
        ...
        public static string XmlPost(string url, string ss)
        {
            ...
            try
            {
                ...
                HttpWebRequest request = WebRequest.Create(url) as HttpWebRequest;//Casting
                                                                                  //Casting = tip dönüştürme
                                                                                  //ConvertToHttpWebRequest(WebRequest.Create(url))
                request.Method = "POST";
